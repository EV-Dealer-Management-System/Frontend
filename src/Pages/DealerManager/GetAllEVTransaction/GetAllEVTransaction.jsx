...
import { Table, Card, Tag, Statistic, Row, Col, Input, DatePicker, Select, Button, message, Space, Tooltip } from 'antd';
import { SearchOutlined, ReloadOutlined, DollarOutlined, TransactionOutlined, CalendarOutlined } from '@ant-design/icons';
...
function GetAllEVTransactions() {
    ...
    const calculateStatistics = (data) => {
        ...
        const successfulTransactions = data.filter(item => item.status === 1).length;
    ...
    const getProviderTag = (provider) => {
        ...
        return <Tag color={colors[provider] || 'default'}>
            ...
            >
                ...
                >
                    <Table
                        ...
                        size="middle"
                        rowClassName={(record) =>
                            `hover:bg-blue-50 transition-colors cursor-pointer ${record.status === 1 ? 'bg-green-50' :
                                record.status === -1 ? 'bg-red-50' : 'bg-yellow-50'
                            }`
                        }
                        locale={{
