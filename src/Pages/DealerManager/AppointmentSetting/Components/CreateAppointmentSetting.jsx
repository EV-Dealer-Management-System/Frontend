...
const CreateAppointmentSetting = ({ onSuccess, onCancel }) => {
    ...
    const checkExisting = async () => {
      try {
        ...
        if (res?.isSuccess && res?.result) {
          ...
          message.warning(
            "Đại lý đã có AppointmentSetting. Không thể tạo mới, vui lòng cập nhật."
          );
        }
      } catch {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const res = await CreateAppointmentSettingApi.createAppointmentSetting(
        values.allowOverlappingAppointments ?? false,
        ...
        values.minIntervalBetweenAppointments ?? 0,
        values.breakTimeBetweenAppointments ?? 0
      );
      if (res?.isSuccess) {
      ...
      message.error(
        error?.response?.data?.message || "Có lỗi khi tạo thiết lập"
      );
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <Card>
        <Title level={4}>
          ...
          Đã có Cài Đặt Lịch Hẹn
        </Title>
        <Text type="danger">
          Đại lý này đã có Cài Đặt Lịch Hẹn. Bạn không thể tạo mới, vui lòng
          dùng chức năng Cập nhật.
        </Text>
