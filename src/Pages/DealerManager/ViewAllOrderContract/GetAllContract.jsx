    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          {/* Debug: Hiển thị status để kiểm tra */}
          {console.log('Record status:', record.status, 'Type:', typeof record.status)}
          
          {record.status === 1 && ( // Chỉ hiện nút duyệt khi status = Draft (1)
          ...
          {/* Temporary: Luôn hiển thị nút duyệt để test */}
          <Tooltip title={`Duyệt hợp đồng (Status: ${record.status})`}>
            <Button
              type="primary"
              size="small"
              icon={<CheckOutlined />}
