...
function TemplateOverview() {
  ...
  const loadAllTemplates = useCallback(async (page = 1, size = 12, search = '') => {
    try {
      setLoading(true);
  ...
  return (
    <EVMStaffLayout>
      <PageContainer
        ...
        subTitle={
          pagination
            ? `${pagination.totalItems} mẫu xe điện (${Array.isArray(templates) ? templates.filter(t => t.isActive === true).length : 0} đang hoạt động)`
            : `${Array.isArray(templates) ? templates.filter(t => t.isActive === true).length : 0} mẫu xe điện có sẵn`
        }
        extra={[
          <Search
            key="search"
